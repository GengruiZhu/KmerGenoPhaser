...
Required:
  ...
  --sample_names     <str>    Comma-separated ALL sample names
    ...
    for TSAMPLE in "${TARGET_ARR[@]}"; do
      ...
      for GNAME in "${GRP_NAME_ARR[@]}"; do
        while IFS= read -r CHROM _; do
          ...
          SRC=""
          for candidate in \
              "${EXISTING_DIAG_DIR}/${CHROM}.${GNAME}.bedgraph" \
              "${EXISTING_DIAG_DIR}/${CHROM}.${GNAME}.bedGraph" \
              "${EXISTING_DIAG_DIR}/${CHROM}.${GNAME^^}.bedgraph" \
              "${EXISTING_DIAG_DIR}/${CHROM}.${GNAME^^}.bedGraph"; do
